...
GrPeo   = [10, 50, 100, 150, 200]; NNPeo = length (GrPeo);
...
rstName = [dirName 'PvsNPeo AI-Batch datIdx=' num2str(datIdx) ' nbeta=' num2str(nbeta) ...
    ' ' OptAlg ' T=' num2str(T) ' alpha=' num2str(alpha) ' maxIter=' num2str(maxIter) ...
...
tsRstCell  = cell (2,2);
meanPolicy = zeros (NGamma, NNPeo);
stdPolicy = zeros (NGamma, NNPeo);
meanValue = zeros (NGamma, NNPeo);
stdValue  = zeros (NGamma, NNPeo);
...
if ~exist (rstName, 'file')
    for na = 1 : NNPeo
        ...
        GrTheta = zeros( md.Lgo, NPeo, NGamma);
...
for ng = 1 : NGamma
    ...
    errorbar (GrPeo, iAvgRwd, iStdRwd, sevenColors{ng}, ...
        'LineWidth', 2, 'MarkerSize', 6);
    hold on;
end
xlim ([5, 205]);
