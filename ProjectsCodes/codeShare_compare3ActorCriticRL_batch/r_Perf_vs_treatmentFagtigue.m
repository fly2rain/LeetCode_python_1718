...
GrGamma = [0, 0.3, 0.6, 0.95, 1];  NGamma=length(GrGamma);
GrTau = 0 :  0.2 : 0.8;  NTau = length (GrTau);

%% data #6@4 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%% the parameter we use, 05/12/2016 %%%%%%%%%%
% NPeo=50; T=100;
datIdx=6; nbeta=4; NPeo=50; T=100; md.Lo=10; maxIter=30; nseRwd=1; nseSt=1; 
...
rstName = [dirName 'PvsTreatmentFatigue AI-Batch datIdx=' num2str(datIdx) ' nbeta=' num2str(nbeta) ...
...
GrTheta = zeros( md.Lgo, NPeo, NGamma, NTau);
GrW     = zeros( md.Lho, NPeo, NGamma, NTau);
% (1,1) meanVal, (1,2) stdVal, (2,1) meanPol, (2,2) stdPol
tsRstCell  = cell (2,2);
meanPolicy = zeros (NGamma, NTau);
stdPolicy = zeros (NGamma, NTau);
meanValue = zeros (NGamma, NTau);
stdValue  = zeros (NGamma, NTau);
...
if ~exist (rstName, 'file')
    for nt = 1 : NTau
        ...
        parfor ng = 1 : NGamma
            ...
            meanPolicy (ng, nt) = mean (NLongRwd);
            stdPolicy  (ng, nt) = std (NLongRwd) ./ sqrtNPeo * 2;
            meanValue(ng, nt) = mean (rmseVal);
            stdValue (ng, nt) = std (rmseVal) ./ sqrtNPeo * 2;
        end
        algTime = toc / 60;
...
for ng = 1 : NGamma
    ...
    errorbar (GrTau, iAvgRwd, iStdRwd, sevenColors{ng}, ...
        'LineWidth', 2, 'MarkerSize', 6);
    hold on;
end
xlim ([-0.03, 0.83]);
