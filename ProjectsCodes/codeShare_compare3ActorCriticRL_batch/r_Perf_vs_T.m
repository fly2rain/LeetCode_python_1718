...
GrT = [20, 42, 100, 200, 400, 600];   NT = length (GrT);
...
% GrT = [20, 42, 100, 200, 400, 600];   NT = length (GrT);
% datIdx=6; nbeta=4; NPeo=20; T=200; md.Lo=10; maxIter=30; nseRwd=1; nseSt=1; 
% OptAlg=GrAlgOPT{2}; dirName=''; rngVal=10; valBnd=10; zetaA=0.05; alpha=0;
% zetaC=0.05; order=1; FoChoice='itself'
% % NPeo=50 ---------------
% datIdx=6; nbeta=4; NPeo=50; T=200; md.Lo=10; maxIter=30; nseRwd=1; nseSt=1; 
% OptAlg=GrAlgOPT{2}; dirName='/'; rngVal=10; valBnd=10; zetaA=0.05; alpha=0;
...
rstName = [dirName 'PvsT AI-Batch datIdx=' num2str(datIdx) ' nbeta=' num2str(nbeta) ...
...
GrTheta = zeros( md.Lgo, NPeo, NGamma, NT);
GrW     = zeros( md.Lho, NPeo, NGamma, NT);
% (1,1) meanVal, (1,2) stdVal, (2,1) meanPol, (2,2) stdPol
tsRstCell  = cell (2,2);
meanPolicy = zeros (NGamma, NT);
stdPolicy = zeros (NGamma, NT);
meanValue = zeros (NGamma, NT);
stdValue  = zeros (NGamma, NT);
...
cmpItem = 'T';
xlimRange = [GrT(1)*0.98, GrT(end)*1.02];
...
for ng = 1 : NGamma
    ...
    errorbar (GrT, iAvgRwd, iStdRwd, sevenColors{ng}, ...
        'LineWidth', 2, 'MarkerSize', 6);
    hold on;
end
xlim (xlimRange);
