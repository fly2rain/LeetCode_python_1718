...
GrGamma = [0, 0.3, 0.7, 0.95, 1];  NGamma=length(GrGamma); alpha=0;
GrAlpha = [0, 0.25, 0.5, 0.75, 1];   NAlpha = length (GrAlpha);

%% data #6@4 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%% the parameter we use, 05/12/2016 %%%%%%%%%%
% % T = 200
datIdx=6; nbeta=4; NPeo=20; T=200; md.Lo=10; maxIter=30; nseRwd=1; nseSt=1; 
OptAlg=GrAlgOPT{2}; dirName=''; rngVal=10; valBnd=10; zetaA=0.05; 
...
GrBeta6 = [
    ...
    0.4,0.3,0.4,0.7,0.25,0.6,0.25,3,0.25,0.25,0.4,0.1,0.5, 0, 0, 0, 0, 500;%6
    0.4,0.3,0,0.7,0.05,0.6,0.25,3,0.25,0.25,0.4,0.1,0.5, 0, 0, 0, 0, 500;%7
    0.4,0.3,0.4,0.7,0.0,0.,0.25,3,0.25,0.25,0.4,0.1,0.5, 0, 0, 0, 0, 500;%8
    ]; % only useful on
...
rstName = [dirName 'PvsAlpha AI-Batch datIdx=' num2str(datIdx) ' nbeta=' num2str(nbeta) ...
    ' ' OptAlg ' NPeo=' num2str(NPeo) ' T=' num2str(T) ' maxIter=' num2str(maxIter) ...
...
GrTheta = zeros( md.Lgo, NPeo, NGamma, NAlpha);
GrW     = zeros( md.Lho, NPeo, NGamma, NAlpha);
% (1,1) meanVal, (1,2) stdVal, (2,1) meanPol, (2,2) stdPol
tsRstCell  = cell (2,2);
meanPolicy = zeros (NGamma, NAlpha);
stdPolicy = zeros (NGamma, NAlpha);
meanValue = zeros (NGamma, NAlpha);
stdValue  = zeros (NGamma, NAlpha);
...
if ~exist (rstName, 'file')
    for na = 1 : NAlpha
        ...
        parfor ng = 1 : NGamma
            ...
            meanPolicy (ng, na) = mean (NLongRwd);
            stdPolicy  (ng, na) = std (NLongRwd) ./ sqrtNPeo * 2;
            meanValue(ng, na) = mean (rmseVal);
            stdValue (ng, na) = std (rmseVal) ./ sqrtNPeo * 2;
        end
        algTime = toc / 60;
...
for ng = 1 : NGamma
    ...
    errorbar (GrAlpha, iAvgRwd, iStdRwd, sevenColors{ng}, ...
        'LineWidth', 2, 'MarkerSize', 6);
    hold on;
end
xlim ([-0.02, 1.02]);
