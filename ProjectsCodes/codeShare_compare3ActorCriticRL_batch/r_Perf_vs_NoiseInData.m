...
GrNse = 0.1 : 0.5 : 3;   NNse = length (GrNse);

%% data #6 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%% the parameter we use, 05/12/2016 %%%%%%%%%%
% % T=200, NPeo=20
% datIdx=6; nbeta=4; NPeo=20; T=200; md.Lo=10; maxIter=30; nseRwd0=1; nseSt0=1; 
% OptAlg=GrAlgOPT{2}; dirName=''; rngVal=10; valBnd=10; zetaA=0.05; alpha=1;
% zetaC=0.05; order=1; FoChoice='itself';

% % T = 200, NPeo=50
datIdx=6; nbeta=4; NPeo=50; T=200; md.Lo=10; maxIter=30; nseRwd0=1; nseSt0=1; 
OptAlg=GrAlgOPT{2}; dirName=''; rngVal=10; valBnd=10; zetaA=0.05; alpha=1;
zetaC=0.05; order=1; FoChoice='itself';
...
rstName = [dirName 'PvsNse AI-Batch datIdx=' num2str(datIdx) ' nbeta=' num2str(nbeta) ...
    ' ' OptAlg ' NPeo=' num2str(NPeo) ' alpha=' num2str(alpha) ' maxIter=' num2str(maxIter) ...
    ' Lo=' num2str(md.Lo) ' NFea=' num2str(md.Lho) ' zetaC=' num2str(zetaC) ...
    ' zetaA=' num2str(zetaA) '.mat'];

% create the matrices to store the results --
GrTheta = zeros( md.Lgo, NPeo, NGamma, NNse);
GrW     = zeros( md.Lho, NPeo, NGamma, NNse);
% (1,1) meanVal, (1,2) stdVal, (2,1) meanPol, (2,2) stdPol
tsRstCell  = cell (2,2);
meanPolicy = zeros(NGamma, NNse);
stdPolicy = zeros (NGamma, NNse);
meanValue = zeros (NGamma, NNse);
stdValue  = zeros (NGamma, NNse);
...
for ng = 1 : NGamma
    ...
    errorbar (GrNse, iAvgRwd, iStdRwd, sevenColors{ng}, ...
        'LineWidth', 2, 'MarkerSize', 6);
    hold on;
end
xlim ([0, 2.7]);
