...
% gamma:    (1 x 1) a known discount factor. gamma=0 is the contextual bandit,
% 0<gamma<1 is the discount rewardmethod, gamma=1 is the average reward method.
...
% zetaA, zetaC:  (1 x 1) is the strength of \ell_{2}-constraint for actor and critic update respectively
...
% w:    (Lho x 1) the learnt parameter for the value funciton.
%
% References:
...
elseif gamma == 1  % the average reward method ============================
    for iter = 1 : maxIter
        ...
        % %%%% actor updateS  %%%%
        myfun = @(theta) F_actorUpdate_avg ( theta, w, YG, YY0, YY1, zetaA, md.Lgo );
        if (strcmpi (OptAlg, 'GD')) % the Gradient Descent Method (GD)
            theta = F_GradDescent (myfun, theta);
        else  % the fmincon & fminunc             
            [theta,f,exitflag,output] = fmincon (myfun,theta,[],[],[],[],lb,ub,[],opts);
