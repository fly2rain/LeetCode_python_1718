function [theta, w] = F_onlineRL_warmSt (GrO0, np, datIdx, a0a, r0r, X0, Y0, varargin)
% Breif:
...
[gamma,TT,TT0,theta,md,zetaC,zetaA,OptAlg,valBnd,RBFbnd,beta,nseRwd,nseSt,w,dispRst] = parse_opt...
    (varargin,'gamma',0,'TT',-1,'TT0',1,'theta',[],'mydim',[],'zetaC',0.1,'zetaA',0.1, ...
...
if gamma < 1 && gamma >=0 % the contextual bandit & the discount reward method
    ...
    wDenom = zetaC*eye(Lho) + 1/(T+1) * ( XXA*(XXA-gamma*YYA).' + ...
        (1/NT0)*(X0XA*(X0XA-gamma*Y0YA).') );
    wNumer = 1/(T+1) * ( XXA*rrr + (1/NT0)*(X0XA*r0r) );
elseif gamma == 1  % the average reward method ============================
    XXAvg  = XXA - repmat (mean(XXA,2), [1, T]);
    X0XAvg = X0XA - repmat (mean(X0XA,2), [1, NT0]);
    % critic update for wt
    wDenom = zetaC*eye(Lho) + 1/(T+1) * ( XXAvg*(XXA-gamma*YYA).' + ...
        (1/NT0)*( X0XAvg*(X0XA-gamma*Y0YA).') );
    wNumer = 1/(T+1) * ( XXAvg*rrr + (1/NT0)*(X0XAvg*r0r) );
end
wt = wDenom \ wNumer;
end
...
f  = -1/(T+1) * ( ...
    ...
    (1/NT0)*sum( (Q0_A0.*Pi0_0) + (Q0_A1.*Pi0_1), 2 ) ...
    );
...
if nargout > 1 % the gradient g is required
    ...
    g = - 1/(T+1) * ( ....
        ...
        (1/NT0) * sum( X0G .* repmat( (Q0_A0.*dPi0_0)+(Q0_A1.*dPi0_1), Lgo, 1), 2 ) ...
        );
    g = g + zetaA*theta;
end
end