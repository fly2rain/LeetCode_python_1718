function [U, V, objErrs] = gnmf_L1in2(X, varargin)
% -- function [U, V, objErrs] = gnmf_L1in2(X, varargin)
...
[errTol, maxIter, lambda, alpha, weightType, sigma, winSize, normType, normW, verb, ...
    dims, U, V, epss, IDX, percent] = parse_opt(varargin, 'errTol', 0, 'maxIter', 200, 'lambda', 0, 'alpha', 0, ...
               'weightType', 1, 'sigma', 0.5, 'winSize', 1, 'normType', 1, ...
               'normW_orNot', 1, 'verb', 1, 'dimOfData', [], 'W0', [], 'H0^T', [] ,...
               'myeps', 1e-20, 'IDX', [], 'percent', []);
           nSmp = size(X, 2);
...
if lambda > 0 %  when lambda > 0，need to calculate weight matrix   
    switch weightType
        ...
        case 5 % LTSA
            [ri, ci, val] = mexWeightGauss_cluster(X, dims, IDX, winSize, sigma, percent); 
        case 6 % 
...
for t = 1 : maxIter
    ...
    if alpha > 0
        V_sqrt = max(V, epss) .^ (-0.5);
        V_sqrt(V < 1E-3) = 0;
        VUU = VUU + 0.5 * alpha * V_sqrt;
    end
