//! ref: Enhancing Spectral Unmixing by Local Neighborhood Weights
#include <cstdlib>
#include <string>
#include <cmath>
#include <vector>
    ...
    for(int j=winSize; j != ncol-winSize; j++) {
        for(int i=winSize; i != nrow-winSize; i++) {
            ...
            for(int ni=0; ni != nebSize; ni++) {
                ...
                //! the dist defined as <Enhancing Spectral Unmixing by Local Neighborhood Weights> does
                dist = dist / std::sqrt (std::max(dist1 * dist2, 1e-7));

                // calculate the acos (SAD) distance between neigboring pixels
                dist = std::acos (dist);

                // calculate the dist. between neigboring pixels by considering pixel and location distance.
                dist = 1.0 / std::sqrt (dist + neb_locDist[ni] + 1e-7);
