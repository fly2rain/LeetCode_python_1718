function [U, V, objErrs] = gnmf_Multi_new(X, varargin)
% -- function [U, V, objErrs] = gnmf_Multi1(X, varargin)
...
[errTol, maxIter, lambda, weightType, sigma, winSize, normType, normW, verb, ...
    dims, U, V, epss, IDX, percent] = parse_opt(varargin, 'errTol', 0, 'maxIter', 200, 'lambda', 1,  ...
               'weightType', 1, 'sigma', 0.5, 'winSize', 1, 'normType', 1, ...
               'normW_orNot', 1, 'verb', 1, 'dimOfData', [], 'W0', [], 'H0^T', [] ,...
               'myeps', 1e-20, 'IDX', [], 'percent', 10);
nSmp = size(X, 2);
...
if lambda > 0 %  when lambda > 0，need to calculate weight matrix   
    switch weightType
        ...
        case 6 % sad cluster percent
            [ri, ci, val] = mexWeightSad_cluster(X, dims, IDX, winSize, sigma, percent); 
