function [W, H, objErrs] = gnmf_LLE_L1in2(V, varargin)
...
[errTol, maxIter, lambda, weightType, epsilon, winSize, normType, normW, verb, ...
    dims, W, H, epss, alpha] = parse_opt(varargin, 'errTol', 0, 'maxIter', 200, 'lambda', 1,  ...
               'weightType', 1, 'epsilon', 0.5, 'winSize', 1, 'normType', 1, ...
               'normW_orNot', 1, 'verb', 1, 'dimOfData', [], 'W0', [], 'H0', [] ,...
               'epss', 1e-20, 'alpha', 0.2);
% verb: if verb printf;
%       else  non printf;
...
for t = 1 : maxIter
    ...
    if lambda > 0
        regErrs1 = sum(sum((HR-HP).*H));
    else
        regErrs1 = 0;
    end
    
    if alpha > 0
        regErrs2 = alpha * sum(sum(H.^0.5));
    else
        regErrs2 = 0;
    end
    ...
    if(verb && rem(t, verb) == 0)
        ...
        fprintf('[%d] : eucErrs: %f  regErr1: %f  regErrs2: %f  objErrs: %f  relErrs: %f\n', ...
